...
function updateUILanguage() {
    ...
    if (h1) h1.textContent = isEnglish ? 'Lateral Thinking' : 'pensée latérale';
    ...
    if (instructionText) {
        instructionText.textContent = isEnglish 
            ? 'What are we going to make with these elements: toilet paper, paint and hair? In addition to what we have already stolen previously.... And what will it be used for?'
            : 'Qu\'allons-nous bien pouvoir fabriquer avec ces éléments : du pepier-toilette. de la peinture et des cheveux ? En plus de ce que nous avons déjà dérobé précédensent.... Et à quoi cela servira-t-il ?';
    }

    const objectiveText = document.querySelectorAll('.audio-section p')[1];
    if (objectiveText) {
        objectiveText.textContent = isEnglish 
            ? 'answer the question written on the card.'
            : 'rependre à la question inscrites sur la carte.';
    }
