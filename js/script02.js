...
function updateUILanguage() {
    ...
    if (h1) h1.textContent = isEnglish ? 'Watch your ID number!' : 'Gare à ton matricule!';
    ...
    if (instructionText) {
        instructionText.textContent = isEnglish 
            ? 'Upon arrival on the island, each of the seven criminals receives an ID number and a cell number. Tony Truand, with an IQ of 133 - and this last unit is not without importance - spots a hidden message: could he have a friend in the administration, a potential accomplice?'
            : 'À son arrivée sur l\'île, chacun des sept malfrats reçoit un matricule et un numéro de cellule. Tony Truand, au QI de 133 - et cette dernière unité n\'est pas sans importance - y distingue un message caché : aurait-il un ami dans l\'administration, un complice potentiel ?';
    }

    const objectiveText = document.querySelectorAll('.audio-section p')[1];
    if (objectiveText) {
        objectiveText.textContent = isEnglish 
            ? 'find the sign spotted by Tony Truand when reading the ID numbers and cell numbers.'
            : 'trouver le signe apeçu par Tony Truand à la lecture des matricules et numeros de cellule.';
    }
    ...
    if (hintText) {
        hintText.textContent = isEnglish 
            ? 'Mix the ID numbers, this could show you the exit.'
            : 'Melanger les matricules cela poura vous mentrer la sortie.';
    }
    ...
    prisonerLists.forEach(list => {
        ...
        items.forEach(item => {
            item.addEventListener('click', function() {
                ...
                if (hintText) {
                    hintText.textContent = isEnglish
                        ? "Hint found! Look carefully at the pattern in the letters."
                        : "Indice trouvé! Regardez attentivement le motif dans les lettres.";
                }
