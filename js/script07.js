...
function updateUILanguage() {
    ...
    if (h1) h1.textContent = isEnglish ? 'Letters and the Void' : 'lettres et le Néant';
    ...
    if (instructionText) {
        instructionText.textContent = isEnglish 
            ? '"Phew! What a puzzle. We need to get our heads around it. It\'s time for the refectory: for me it will be Vol-vent and emmental, for lunch!"'
            : '« Pfiou ! Quel casse-tête. Il faut qu\'on s\'aire un peu la tête 。 C\'est l\'heure du réfectoire : pour moi ce sera Vol-vent et emmental, ce midi !';
    }

    const objectiveText = document.querySelectorAll('.audio-section p')[1];
    if (objectiveText) {
        objectiveText.textContent = isEnglish 
            ? 'find the hidden message in this text.'
    ...
    if (hintText) {
        hintText.textContent = isEnglish 
            ? 'These letters will form 4 words.'
            : 'Ces lettres constitueront 4 mots.';
    }
