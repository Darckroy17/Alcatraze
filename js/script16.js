...
function updateUILanguage() {
    ...
    if (h1) h1.textContent = isEnglish ? 'Looks like the south' : 'On dirait le sud';
    ...
    if (instructionText) {
        instructionText.textContent = isEnglish 
            ? 'Tony now knows how to reach the roof and not stay stuck in the ventilation shaft. Once on the roof, he must use gutters but doesn\'t know in which direction. In Al Capone\'s notebook, he finds other elements he hasn\'t exploited yet.'
            : 'Tony sait maintenant comment atteindre le toit et ne pas rester coir dans la colonne d\'aération, une fois sur le toit il dois emprunter des gouttières mais il ne savent pas dans quelle direction. Dans le cahier d\'Al Capone, il trouve d\'autres éléments qu\'il encore exploités.';
    }

    const objectiveText = document.querySelectorAll('.audio-section p')[1];
    if (objectiveText) {
        objectiveText.textContent = isEnglish 
            ? 'Find how to escape without being seen, once on the roof.'
            : 'Trouver comment s\'échaper sans etre vue, une fois sur le toit.';
    }

    const cheminiText = document.querySelector('.image-chemin p');
    if (cheminiText) {
        cheminiText.textContent = isEnglish 
            ? 'Only one way out: have the south at your back without being in the west! and the wind will turn.'
            : 'Une seule issue : avoir le sud au dos sans être à l\'ouest ! et le vent tournera.';
    }
    ...
    const choiceTitles = [
        isEnglish ? 'Direction #1' : 'Direction #1',
        isEnglish ? 'Direction #2' : 'Direction #2',
        isEnglish ? 'Direction #3' : 'Direction #3',
        isEnglish ? 'Direction #4' : 'Direction #4',
        isEnglish ? 'Direction #5' : 'Direction #5',
        isEnglish ? 'Direction #6' : 'Direction #6',
        isEnglish ? 'Direction #7' : 'Direction #7',
        isEnglish ? 'Direction #8' : 'Direction #8'
    ];
    ...
    const translations = {
        fr: ['Nord', 'Sud', 'Est', 'Ouest', 'SE', 'NO', 'SO', 'NE'],
        en: ['North', 'South', 'East', 'West', 'SE', 'NW', 'SW', 'NE']
    };
    ...
    choiceButtons.forEach(button => {
        button.textContent = isEnglish ? 'Choose this grid' : 'Choisir cette grille';
    });
