...
function updateUILanguage() {
    ...
    if (h1) h1.textContent = isEnglish ? 'Corridor noise' : 'Bruits de couloir';
    ...
    if (instructionText) {
        instructionText.textContent = isEnglish 
            ? 'The strategy is clear for Tony: steal spoons from the refectory, sharpen them, use them to enlarge the dimensions of each ventilation grille, then cover this grille with a cardboard reproduction so that the guards see nothing but fire. "I understand better the diagrams in Al Capone\'s notebook, now. We clearly see this gallery one meter wide and the direction to get out."'
            : 'La stratégie est claire pour Tony : voler des cuillères au réfectoire, les aiguiser, s\'en servir pour agrandir les dimensions de chaque grille d\'aé-ration, puis recouvrir cette grille par une reproduction en carton pour que les gardiens n\'y voient que du feu. «Je comprends mieux les schémas du carnet d\'Al Capone, maintenant. On voit clairement cette galerie large d\'un mètre et le sens pour s\'en sortir. »';
    }

    const objectiveText = document.querySelectorAll('.audio-section p')[1];
    if (objectiveText) {
        objectiveText.textContent = isEnglish 
            ? 'Interpret the diagrams from Al Capone\'s notebook.'
            : 'Interpréter les schémas du carnet d\'Al Capone.';
    }
    ...
    const translations = {
        fr: [
            '1.peinture et cheveux',
            '2.cheveux et papier',
            '1.papier-toilette',
            '2.peinture et cheveux',
            '1.papier et cheveux',
            '2.toilette et cheveux'
        ],
        en: [
            '1.paint and hair',
            '2.hair and paper',
    ...
    if (hintText) {
        hintText.textContent = isEnglish 
            ? 'All these letters = 4 words.'
            : 'tout ces lettres = 4 mots.';
    }
